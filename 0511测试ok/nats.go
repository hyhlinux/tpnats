...
func (o Options) Connect() (*Conn, error) {
	...
	go nc.asyncDispatch()
	fmt.Println("func (o Options) Connect() (*Conn, error) end")
	return nc, nil
}
...
func (nc *Conn) spinUpGoRoutines() {
	...
	fmt.Println("go nc.readLoop(),go nc.flusher()")
	go nc.readLoop()
	go nc.flusher()
...
func (nc *Conn) processConnectInit() error {
	...
	fmt.Println("func (nc *Conn) processConnectInit() error end")
	return nil
}
...
func (nc *Conn) connect() error {
	...
	for i := range nc.srvPool {
		...
		fmt.Println("Main connect function,nc.createConn()")
...
func (nc *Conn) connectProto() (string, error) {
	...
	cinfo := connectInfo{o.Verbose, o.Pedantic,
		user, pass, token,
		o.Secure, o.Name, LangString, Version, DevId}
...
func (nc *Conn) doReconnect() {
	...
	for len(nc.srvPool) > 0 {
		...
		fmt.Println("doReconnect() function")
		err = nc.createConn()
...
func (nc *Conn) processOpErr(err error) {
	fmt.Println("func (nc *Conn) processOpErr(err error) start")
	nc.mu.Lock()
	...
	if nc.Opts.AllowReconnect && nc.status == CONNECTED {
		...
		fmt.Println("func (nc *Conn) processOpErr(err error) end  --->go nc.doReconnect()  ")
		go nc.doReconnect()
		nc.mu.Unlock()
		fmt.Println("func (nc *Conn) processOpErr(err error) end")
		return
	}
...
func (nc *Conn) asyncDispatch() {
	...
	for {
		fmt.Println("func (nc *Conn) asyncDispatch() ")
		if f, ok := <-ach; !ok {
			return
		} else {
			f()
...
func (nc *Conn) readLoop() {
	...
	fmt.Println("func (nc *Conn) readLoop() end")
...
func (nc *Conn) flusher() {
	...
	for {
		fmt.Println("func (nc *Conn) flusher() read from _, ok := <-fch;")
		if _, ok := <-fch; !ok {
			return
		}
		fmt.Println("func (nc *Conn) flusher() read from _, ok := <-fch; end")
		nc.mu.Lock()
	...
	fmt.Println("func (nc *Conn) flusher() end")
}
